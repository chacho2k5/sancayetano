                ...
                <div class="table-scroll">
                    <table class="table table-hover table-bordered table-responsive-md text-nowrap my-table table-sticky">
                        ...
                        <tbody class="my-body-table">
                            ...
                            <tr id={{ $reg->id }} 
                                    x-data="{ showProduct_ + $el.id: false }"
                                    x-on:click="alert(showProduct_+$el.id),showProduct_{{ $reg->id }}=!showProduct_{{ $reg->id }}, selectedRow=showProduct_{{ $reg->id }}, auxId=$el.id, auxEstado={{ $reg->estado_id }}, auxReclamo={{ $reg->reclamo }}" 
                                    x-bind:class="{'bg-success text-white':$el.id == auxId && showProduct_{{ $reg->id }}, 'bg-primary text-white':{{ $reg->estado_id }} == 4, 'bg-warning':{{ $reg->reclamo }}}">
                                <td x-bind:class="{'bg-primary text-white':{{ $reg->estado_id }} == 4, 'bg-warning':{{ $reg->reclamo }}}">
                                    {{ ($reg->estado_id==2) ? '*' : ' ' }}
                                </td>
