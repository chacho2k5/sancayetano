<div> 
    <div x-data="{selectedRow: @entangle('selectedRow'), auxId: @entangle('registro_id'), auxEstado: @entangle('estado_id') }" class="px-0 mx-0 container-fluid">
        <div class="mb-1 row row-cols-auto justify-content-between">
            ...
            <div class="col">
                ...
                <button wire:click.prevent="reclamoModal()" class="text-white btn btn-warning btn-sm ps-1" data-toggle="tooltip" title='Agregar Reclamo.'>
                    <i class="bi bi-plus-circle pe-1 ps-1"></i>
                    Reclamo
                </button>
                {{-- Agregar OT --}}
                <button wire:click="editOT('create')" class="btn btn-info btn-sm" data-toggle="tooltip" title='Nueva OT.'>
                    <i class="bi bi-plus-circle text-outline-outline-success text-bold text-md-center"></i>
                </button>
                {{-- Mostrar OT --}}
                <button wire:click="editOT('show')" class="btn btn-success btn-sm" data-toggle="tooltip" title='Mostrar datos.'>
                    <i class="bi bi-eye text-outline-success text-bold text-md-center"></i>
                </button>
                {{-- Editar OT --}}
                <button wire:click="editOT('edit')" class="btn btn-primary btn-sm" data-toggle="tooltip" title='Editar OT.'>
                    <i class="bi bi-pencil-fill text-outline-primary text-bold text-md-center"></i>
                </button>
                {{-- Eliminar OT --}}
                <button wire:click="deleteModal()" class="btn btn-danger btn-sm" data-toggle="tooltip" title='Borrar OT'>
                ...
                <div class="table-scroll">
                    <table class="table table-hover table-bordered table-responsive-md text-nowrap my-table table-sticky">
                        ...
                        <tbody class="my-body-table">
                            ...
                            <tr id={{ $reg->id }} x-on:click="auxId=$el.id, auxEstado={{ $reg->estado_id }}, selectedRow = (selectedRow === $event.target.parentElement ? null : $event.target.parentElement)", x-bind:class="{'bg-success text-white':selectedRow === $event.target.parentElement && $el.id == auxId, 'bg-primary text-white':{{ $reg->estado_id }} == 4, 'bg-warning':{{ $reg->reclamo }}}">
                            {{-- <tr id={{ $reg->id }} x-on:click="auxId=$el.id"> --}}
