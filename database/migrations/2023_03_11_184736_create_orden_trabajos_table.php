...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('orden_trabajos', function (Blueprint $table) {
            ...
            // $table->unsignedBigInteger('articulo_id')->nullable()->constrained();      // Id del articulo
            // $table->string('articulo_nombre',100)->nullable();       // Nombre del trabajo
            $table->double('ancho')->nullable();        
            $table->double('largo')->nullable();        
            $table->double('espesor')->nullable();      // Este espesor determina si es AD/BD ??
            // $table->string('ale')->nullable(); // ancho/largo/espesor
            ...
            $table->boolean('trabajo_activo')->nullable()->default(false);
            ...
            // $table->string('numero_ot')->virtualAs("concat(EXTRACT(DAY FROM fecha_pedido),' ',EXTRACT(MONTH FROM fecha_pedido),' ',EXTRACT(YEAR FROM fecha_pedido)");
            // $table->string('numero_ot')->virtualAs("concat(LPAD(EXTRACT(DAY FROM fecha_pedido),2,0),EXTRACT(MONTH FROM fecha_pedido),EXTRACT(YEAR FROM fecha_pedido), \'-\', ot_mensual)");
            // $table->string('numero_ot')->virtualAs("concat(LPAD(EXTRACT(DAY FROM fecha_pedido),2,'0'))");
            // $table->string('full_mes')->virtualAs('concat(nombre, \' - A:\', orden, \' L:\', orden, \' E:\', orden)');
